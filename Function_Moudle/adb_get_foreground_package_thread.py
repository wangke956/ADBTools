        ...
        try:
            ...
            if check_result.returncode != 0:
                if "device not found" in check_result.stderr.lower():
                    self.error_signal.emit(f"设备 {self.device_id} 未连接，请检查设备连接状态")
                elif "offline" in check_result.stderr.lower():
                    self.error_signal.emit(f"设备 {self.device_id} 处于离线状态，请重新连接")
                else:
                    self.error_signal.emit(f"ADB连接失败: {check_result.stderr}")
                return
            ...
            if result.returncode != 0:
                ...
                if simple_result.returncode == 0 and simple_result.stdout.strip():
                    ...
                    self.error_signal.emit("无法获取前台应用信息，可能设备不支持或应用权限不足")
                    return
            else:
            ...
            if focus_info and 'mCurrentFocus' in focus_info:
                try:
                    ...
                    self.error_signal.emit("解析应用信息失败")
            else:
                self.result_signal.emit("当前没有正在运行的应用")
                
        except subprocess.CalledProcessError as e:
            self.error_signal.emit(f"ADB命令执行失败: {str(e)}")
