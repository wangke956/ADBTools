...
class AppOperationsManager:
    ...
    def list_package(self):
        ...
        log_button_click("list_package_button", "列出包名")

        if device_id in devices_id_lst:
            try:
                if self.main_window.connection_mode == 'u2' and self.main_window.d:
                    from Function_Moudle.list_package_thread import ListPackageThread
                    self.main_window.list_package_thread = ListPackageThread(self.main_window.d)
                elif self.main_window.connection_mode == 'adb':
                    from Function_Moudle.adb_list_package_thread import ADBListPackageThread
                    self.main_window.list_package_thread = ADBListPackageThread(device_id)
                else:
                ...
                self.main_window.list_package_thread.result_signal.connect(self.textBrowser.append)
    ...
    def show_force_stop_app_dialog(self):
        ...
        if device_id in devices_id_lst:
            ...
            if ok and package_name:
                self.main_window.logger.info(f"强制停止应用: {package_name}")
    ...
    def show_clear_app_cache_dialog(self):
        ...
        if device_id in devices_id_lst:
            ...
            if ok and package_name:
                self.main_window.logger.info(f"清除应用缓存: {package_name}")
                ...
                if reply == QMessageBox.Yes:
                    ...
                    self.main_window.logger.info("用户取消清除缓存操作")
            else:
                self.main_window.logger.info("用户取消输入或输入为空")
        else:
            log_method_result("show_clear_app_cache_dialog", False, "设备未连接")
    ...
    def aapt_getpackage_name_dilog(self):
        ...
        if apk_path:
            self.main_window.logger.info(f"选择APK: {apk_path}")
    ...
    def view_apk_path_wrapper(self):
        ...
        log_button_click("view_apk_path_button", "查看APK路径")

        if device_id in devices_id_lst:
            package_name, ok = QInputDialog.getText(
                self.main_window, "输入包名", "请输入要查看APK路径的应用包名："
            )
            if ok and package_name:
                self.main_window.logger.info(f"查看APK路径: {package_name}")
                ...
                try:
                    ...
                    log_method_result("view_apk_path_wrapper", True, f"查看APK路径线程已启动: {package_name}")
                except Exception as e:
                    log_method_result("view_apk_path_wrapper", False, str(e))
                    self.textBrowser.append(f"启动查看APK路径线程失败: {e}")
            else:
                self.main_window.logger.info("用户取消输入或输入为空")
        else:
    ...
    def app_version_check(self):
        ...
        log_button_click("app_version_check_button", "应用版本检查")

        if device_id in devices_id_lst:
            try:
                if self.main_window.connection_mode == 'u2' and self.main_window.d:
                    from Function_Moudle.app_version_check_thread import AppVersionCheckThread
                    self.main_window.app_version_thread = AppVersionCheckThread(
                        self.main_window.d, device_id
                    )
                elif self.main_window.connection_mode == 'adb':
                    from Function_Moudle.app_version_check_thread import AppVersionCheckThread
                    self.main_window.app_version_thread = AppVersionCheckThread(
                ...
                self.main_window.app_version_thread.progress_signal.connect(self.textBrowser.append)
                self.main_window.app_version_thread.result_signal.connect(self.textBrowser.append)
                self.main_window.app_version_thread.error_signal.connect(self.textBrowser.append)
                self.main_window.app_version_thread.start()
