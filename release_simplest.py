...
### 1. 线程模块重构
- 创建线程池管理器（ThreadPoolManager）统一管理线程生命周期
- 实现操作队列管理器（OperationQueueManager）处理任务调度
- 更新线程工厂（ThreadFactory）集成线程池和队列管理
- 创建线程基类（BaseThread）添加状态监控和超时处理
- 整合相似功能线程到同一模块（device_threads, app_threads, file_threads等）

### 2. 文件管理器功能增强
- 添加文件删除确认对话框，防止误删
- 实现文件权限管理（chmod）功能
- 添加常用权限预设按钮（755, 777, 644等）

### 3. 用户体验优化
- 添加操作历史管理器支持撤销/重做功能
- 实现错误提示对话框和操作引导系统
- 在菜单栏添加编辑菜单（撤销、重做、快速入门）
